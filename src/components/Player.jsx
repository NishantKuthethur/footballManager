...
const Player = ({ player, position }) => {
    ...
    return (
      <div style={style} className="player-circle flex flex-col items-center justify-center gap-1 text-texts-heading" onClick={handlePlayerSelection}>
        <div className={`relative h-8 w-8  rounded-full ${isSelected ? 'bg-primary-orange' : 'bg-neutral-2 border-2'} bg-neutral-2 border-2`}><div className="jersey-number flex justify-center text-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 right-0.5">{player["Jersey Number"]}</div></div>
